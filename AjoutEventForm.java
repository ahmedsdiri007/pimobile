...
import com.resto.service.EvenementService;
...
public class AjoutRestoForm extends BaseForm{
    ...
    public AjoutRestoForm(Resources res){
       ...
       swipe.setUIID("Container");
        ...
        if (n % 2 == 0) {
               ...
               n++;
                ...
                n++;
          ListRestoForm a = new ListRestoForm(res);
           a.show();}
            refreshTheme();
       ...
       //
        ...
        nom.setUIID("TestFieldBlack");
        addStringValue("nom",nom);
        
         TextField prenom = new TextField("","entrer prenom !!");
        prenom.setUIID("TestFieldBlack");
        addStringValue("prenom",prenom);
        
         TextField mail = new TextField("","entrer mail !!");
        mail.setUIID("TestFieldBlack");
        addStringValue("mail",mail);
        
       
        
        ComboBox<String> ev= new ComboBox<>();
        EvenementService sc=new EvenementService();
        ArrayList<Evenement> listCat = sc.getListEvenement();
        for(int i=0;i<listCat.size();i++ )
        {
            ev.addItem(listCat.get(i).getNomevent());
        }
                addStringValue("evenement",ev);
       ...
       try {
           ...
           Dialog.show("Veuillez verifier les donnees","", "Annuler", "OK");
               ...
               final Dialog iDialog = ip.showInfiniteBlocking();
                             String id=ev.getSelectedItem();

               reservation_evenement r= new reservation_evenement(nom.getText(),prenom.getText(),mail.getText());
               
               System.out.println("data resto == "+r);
               Message m = new Message("evenement ajouté");

Display.getInstance().sendMessage(new String[] {"ghassen.namouchi@esprit.tn"}, "evenement ajouté", m);
               ReservationService.getInstance().addproduct(r,id);
